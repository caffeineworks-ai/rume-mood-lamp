import { McpAgent } from "agents-sdk";
...
export class RumeMCP extends McpAgent {
  ...
  async init() {
    this.server.tool(
      "rearrange_furniture",
      "침실의 가구(침대, 협탁, TV 테이블)를 새로운 위치로 재배치하고 조명을 초기화합니다.",
      {},
      async () => {
    ...
    this.server.tool(
      "place_lights",
      "발광버섯 조명 군락을 현재 가구 배치에 맞게 새로 배치합니다.",
      {},
      async () => {
